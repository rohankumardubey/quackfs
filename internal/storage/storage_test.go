...
func TestWriteReadActiveLayer(t *testing.T) {
	...
	require.Equal(t, mgr.GetActiveLayerSize(ctx, fileID), uint64(0), "Active layer should be empty")
	require.Nil(t, mgr.GetActiveLayerData(ctx, fileID), "Active layer should be nil")

	input := []byte("hello world")
	err = mgr.WriteFile(ctx, filename, input, 0)
	require.NoError(t, err, "Write error")
	require.Equal(t, mgr.GetActiveLayerSize(ctx, fileID), uint64(len(input)), "Active layer should be the same size as the input")
	require.Equal(t, string(mgr.GetActiveLayerData(ctx, fileID)), string(input), "Active layer should contain the input data")
	...
	fullContent, err := mgr.ReadFile(ctx, filename, 0, fileSize)
	...
	data, err := mgr.ReadFile(ctx, filename, 0, uint64(len(input)))
...
func TestCheckpointingNewActiveLayer(t *testing.T) {
	...
	input2 := []byte("data2")
	err = mgr.WriteFile(ctx, filename, input2, uint64(len(input1)))
	require.NoError(t, err, "Write error")

	data, err := mgr.ReadFile(ctx, filename, uint64(len(input1)), uint64(len(input2)))
	require.NoError(t, err, "GetDataRange error")
...
func TestPartialRead(t *testing.T) {
	...
	data, err := mgr.ReadFile(ctx, filename, 0, partialSize)
	require.NoError(t, err, "GetDataRange error")
...
func TestGetDataRangeByFileName(t *testing.T) {
	...
	readData, err := mgr.ReadFile(ctx, filename, 0, uint64(len(data)))
...
func TestStorageManagerPersistence(t *testing.T) {
	...
	err = mgr.WriteFile(ctx, filename, data1, 0)
	...
	err = mgr.WriteFile(ctx, filename, data2, uint64(len(data1)))
	...
	mgr2, mgr2Cleanup := quackfstest.SetupStorageManager(t)
	defer mgr2Cleanup()

	// Verify the checkpointed data is still correct (should include data1 and data2)
	fileSize2, err := mgr2.SizeOf(ctx, filename)
	require.NoError(t, err, "Failed to get file size")
	fullContent2, err := mgr2.ReadFile(ctx, filename, 0, fileSize2)
	require.NoError(t, err, "Failed to get full content")
	...
	err = mgr2.WriteFile(ctx, filename, data3, uint64(len(data1)+len(data2)))
	require.NoError(t, err, "Failed to write additional data")

	// Verify the combined data is correct in memory
	fileSize3, err := mgr2.SizeOf(ctx, filename)
	require.NoError(t, err, "Failed to get file size")
	fullContent3, err := mgr2.ReadFile(ctx, filename, 0, fileSize3)
	require.NoError(t, err, "Failed to get full content")
	...
	mgr3, mgr3Cleanup := quackfstest.SetupStorageManager(t)
	defer mgr3Cleanup()

	// Verify all checkpointed data persists (should include data1, data2, and data3)
	fileSize4, err := mgr3.SizeOf(ctx, filename)
	require.NoError(t, err, "Failed to get file size")
	fullContent4, err := mgr3.ReadFile(ctx, filename, 0, fileSize4)
	require.NoError(t, err, "Failed to get full content")
...
func TestFuseScenario(t *testing.T) {
	...
	err = mgr.WriteFile(ctx, filename, initialData, 0)
	require.NoError(t, err, "Failed to write initial data")

	// Verify the data
	readData, err := mgr.ReadFile(ctx, filename, 0, uint64(len(initialData)))
	...
	err = mgr.WriteFile(ctx, filename, additionalData, uint64(len(initialData)))
	require.NoError(t, err, "Failed to write additional data")

	// Read the combined data
	combinedData := append(initialData, additionalData...)
	readCombined, err := mgr.ReadFile(ctx, filename, 0, uint64(len(combinedData)))
	...
	readAfterRestart, err := mgr2.ReadFile(ctx, filename, 0, uint64(len(combinedData)))
	...
	err = mgr2.WriteFile(ctx, filename, finalData, uint64(len(combinedData)))
	...
	readFinalData, err := mgr2.ReadFile(ctx, filename, 0, uint64(len(finalCombinedData)))
	...
	mgr3, cleanup3 := quackfstest.SetupStorageManager(t)
	defer cleanup3()

	// Verify only the checkpointed data persists (without the final segment)
	persistedData, err := mgr3.ReadFile(ctx, filename, 0, uint64(len(finalCombinedData)))
...
func TestCalculateVirtualFileSize(t *testing.T) {
	...
	for _, w := range writes {
		err = mgr.WriteFile(ctx, filename, w.data, w.offset)
	...
	err = mgr.WriteFile(ctx, filename, finalData, finalOffset)
	require.NoError(t, err, "Failed to write final data")

	// Get the updated file size
	newSize, err := mgr.SizeOf(ctx, filename)
...
func TestExampleWorkflow(t *testing.T) {
	...
	err = mgr.WriteFile(ctx, filename, data2, expectedOffset2)
	require.NoError(t, err, "Failed to write additional data")
...
func TestWriteToSameOffsetTwice(t *testing.T) {
	...
	err = mgr.WriteFile(ctx, filename, partialData, partialOffset)
	...
	fullContentAfterPartial, err := mgr.ReadFile(ctx, filename, 0, uint64(len(expectedContent)))
	require.NoError(t, err, "Failed to get full content")
...
func TestGetDataRangeWithVersion(t *testing.T) {
	...
	initialContent := []byte("***************")
	err = mgr.WriteFile(ctx, filename, initialContent, 0)
	...
	// Write more content
	updatedContent := []byte("---------------")
	err = mgr.WriteFile(ctx, filename, updatedContent, 0)
...
func TestWithinAndOverlappingWrites(t *testing.T) {
	...
	*/
	...
	err = mgr.WriteFile(ctx, filename, first, 0)
	require.NoError(t, err, "Failed to write first data")

	{
		// check content
		content, err := mgr.ReadFile(ctx, filename, 0, 4096)
	...
	err = mgr.WriteFile(ctx, filename, second, 3000)
	require.NoError(t, err, "Failed to write second data")

	{
		// check content
		content, err := mgr.ReadFile(ctx, filename, 0, 6000)
		require.NoError(t, err, "Failed to read first data")
	...
	err = mgr.WriteFile(ctx, filename, third, 1024)
	...
	err = mgr.WriteFile(ctx, filename, fourth, 2000)
...
func TestReadFileStartingMidChunk(t *testing.T) {
	...
	err = mgr.WriteFile(ctx, filename, secondData, secondOffset)
	require.NoError(t, err, "Failed to write second data")
	...
	data, err := mgr.ReadFile(ctx, filename, readOffset, readSize)
...
func TestConcurrentWrites(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		barrier.Wait()
		mgr.WriteFile(ctx, filename, []byte("hello"), 0)
	}()

	go func() {
		defer wg.Done()
		barrier.Wait()
		mgr.WriteFile(ctx, filename, []byte("world"), 0)
	}()
...
func TestConcurrentReadWrite(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		barrier.Wait()
		content, _ = mgr.ReadFile(ctx, filename, 0, 5)
	}()
...
func TestConcurrentCheckpoint(t *testing.T) {
	...
	err = mgr.WriteFile(ctx, filename, []byte("test data"), 0)
	require.NoError(t, err, "Failed to write data")
